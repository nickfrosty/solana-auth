...
import { MEMO_PROGRAM_ID } from "./auth-transaction.js";
...
export class SolanaAuth {
  ...
  constructor({ message, signedData, overrides }: SolanaAuthConstructorArgs) {
    ...
    if (!!signedData) {
      if (typeof signedData == "string") {
        signedData = JSON.parse(signedData) as unknown as SolanaAuthSignedData;
      }
   ...
   */
  verifySIWS(signedData: SolanaAuthSignedData | undefined = this.signedData) {
